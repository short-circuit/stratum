...
async fn main() -> anyhow::Result<()> {
    ...
    match vault.search("zebra", 10, 0) {
        Ok(res) => {
            ...
            for h in res.results.iter() {
                println!("  hit: {} [{}]", h.page_path, h.content);
            }
        }
    ...
    if idx.exists() {
        for e in std::fs::read_dir(&idx)? {
            println!("  idxfile {}", e?.file_name().to_string_lossy());
        }
    } else {
    ...
    println!(
        "db: {} file: {}",
        dir.path().join(".pkm/blocks.db").exists(),
        dir.path().join("s.md").exists()
    );
    Ok(())
}
