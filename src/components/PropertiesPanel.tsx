...
export default function PropertiesPanel(_props: Props) {
  ...
  return (
    <div className="border-t border-[var(--secondary-200)] dark:border-[var(--secondary-700)] p-3">
      <h3 className="text-xs font-semibold text-[var(--secondary-500)] uppercase mb-2">
        Properties
      </h3>
      ...
      <div className="flex gap-1 mb-2">
        <input
          ...
          className="flex-1 text-xs px-2 py-1 rounded border border-[var(--secondary-300)] dark:border-[var(--secondary-600)] bg-white dark:bg-[var(--secondary-800)]"
        />
        <button
        ...
        <div className="space-y-1">
          {prop.marker && (
            <div className="text-xs flex justify-between">
              <span className="text-[var(--secondary-500)]">Marker</span>
              <span className="font-medium">{prop.marker}</span>
            </div>
          )}
          {prop.priority && (
            <div className="text-xs flex justify-between">
              <span className="text-[var(--secondary-500)]">Priority</span>
          ...
          {prop.properties.map(([k, v], i) => (
            <div key={i} className="text-xs flex justify-between group">
              <span className="text-[var(--secondary-500)]">{k}</span>
              <span className="font-medium">{v}</span>
            </div>
          ))}
          {prop.properties.length === 0 && !prop.marker && (
            <p className="text-xs text-[var(--secondary-400)]">No properties set.</p>
          )}

          {/* Add property */}
          <div className="flex gap-1 mt-2 pt-2 border-t border-[var(--secondary-200)] dark:border-[var(--secondary-700)]">
            <input
              type="text"
              value={newKey}
              onChange={e => setNewKey(e.target.value)}
              placeholder="Key"
              className="w-1/3 text-xs px-1.5 py-0.5 rounded border border-[var(--secondary-300)] dark:border-[var(--secondary-600)] bg-white dark:bg-[var(--secondary-800)]"
            />
            <input
              type="text"
              value={newValue}
