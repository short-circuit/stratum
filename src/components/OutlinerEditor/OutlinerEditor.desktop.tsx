 ...
 *
 * Exact copy of the original index.tsx — kept as a self-contained module
 * so that the desktop experience is completely unchanged.
 *
...
export default function OutlinerEditor({
  ...
  // Wiki-link hover preview delegation + click handling (native click prevents
  // browser navigation)
  useEffect(() => {
    const el = containerRef.current;
    ...
    const handleLinkPrevent = (e: MouseEvent) => {
      // Only prevent browser from following stratum: href — does NOT stop
      // propagation so ProseMirror's handleClick can still fire the
      // links.onClick callback
      const a = (e.target as HTMLElement).closest?.('a');
      if (!a) return;
      const href = a.getAttribute('href');
      if (!href || !isWikiLinkHref(href)) return;
